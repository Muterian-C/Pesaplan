  ...
  const loadData = async () => {
    ...
    try {
      const [summaryData, categoriesData, alertsData] = await Promise.all([
        budgetApi.getSummary(selectedMonth, selectedYear),
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">
                      Spent: {fmt(category.spent)}
                    </span>
                    <span className="text-gray-600 dark:text-gray-400">
                      Budget: {fmt(category.budget)}
                    </span>
                  </div>
                  ...
                  {category.remaining >= 0 ? (
                    <p className="text-xs text-gray-500">
                      Remaining: {fmt(category.remaining)}
                    </p>
                  ) : (
                    <p className="text-xs text-red-600">
                      Over budget by: {fmt(Math.abs(category.remaining))}
                    </p>
                  )}
                </div>
              </motion.div>
