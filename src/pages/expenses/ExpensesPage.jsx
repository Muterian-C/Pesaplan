...
const FILTERS = [
  ...
  ...EXPENSE_CATEGORIES.map(c => ({ id: c.id, label: c.label })),
];

export default function ExpensesPage() {
  const { expenses, refetchData, dataLoading } = useApp();
  ...
  const checkBudgetWarnings = async () => {
    try {
      ...
      const budgetSummary = await budgetApi.getSummary(currentMonth, currentYear);
